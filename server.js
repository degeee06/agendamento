...
async function horarioDisponivel(cliente, data, horario) {
  const { data: agendamentos, error } = await supabase
    ...
    .eq("data", data)
    .eq("horario", horario)
    .in("status", ["pendente", "confirmado"]); // mantém só ativos
  if (error) throw error;
  // horário está livre se não houver agendamento pendente ou confirmado
...
app.get("/disponiveis/:cliente/:data", authMiddleware, async (req, res) => {
  try {
    ...
    // garante que todos os horários tenham o mesmo formato HH:mm
    const ocupados = agendamentos.map(a => a.horario.slice(0,5));
    res.json({ ocupados });
  } catch (err) {
    console.error(err);
    res.status(500).json({ msg: "Erro interno" });
